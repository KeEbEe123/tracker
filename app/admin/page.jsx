...
export default function AdminPage() {
  ...
  if (!session || !ADMIN_EMAILS.includes(session.user?.email)) {
    if (status === "authenticated") {
      return <div className="container mx-auto py-8 px-4 text-center text-red-500">Access Denied</div>;
    }
          ...
          <CardContent className="p-6 min-h-[400px]">
            <TabsContent value="leaderboard" active={tab === "leaderboard"}>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Faculty Leaderboard</h2>
            ...
            <TabsContent value="certificates" active={tab === "certificates"}>
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white">All Certificates</h2>
                <Button onClick={() => {
                  const rows = allCertificates.map(cert => ({
                    ...
                    "Expiry Date": cert.expiryDate ? new Date(cert.expiryDate).toLocaleDateString() : "",
                    ...
                    {allCertificates.length === 0 ? (
                      ...
                      allCertificates.map((cert, idx) => (
                        <tr key={idx} className="border-b last:border-b-0">
                          ...
                          <td className="px-3 py-2">{cert.issuingOrganization}</td>
                          <td className="px-3 py-2">{cert.issueDate ? new Date(cert.issueDate).toLocaleDateString() : ""}</td>
                          <td className="px-3 py-2">{cert.expiryDate ? new Date(cert.expiryDate).toLocaleDateString() : ""}</td>
                          <td className="px-3 py-2">{cert.credentialId}</td>
                          <td className="px-3 py-2">
                            {cert.credentialUrl ? (
                              <a href={cert.credentialUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 underline">
                                Link
                              </a>
                            ) : (
                              ""
            ...
            <TabsContent value="create" active={tab === "create"}>
              <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Create Certification Link</h2>
              <form
                onSubmit={e => {
                  e.preventDefault();
                  if (certLink && certLinkDesc) {
                    setCertLinkList(prev => [...prev, { url: certLink, desc: certLinkDesc }]);
                    setCertLink("");
                    setCertLinkDesc("");
                  }
                }}
              ...
              {certLinkList.length > 0 && (
                <div className="mt-8">
                  <h3 className="text-lg font-medium mb-2">Created Certification Links</h3>
